...
// Einbinden der Models f�r MongoDB Zugriff
...
//Sicherstellen, dass der Zugriff nur get�tigt wird, wenn der Nutzer angemeldet ist
...
module.exports = function(passport){
  ...
  /* Login Post handeln */
    router.post('/login',function(req, res, next) {
        passport.authenticate('login', function(err,user){
            ...
            // falls Login nicht m�glich Weiterleitung zur Login Seite
  ...
  /* Handle Registration POST */
    ...
    router.get('/create_multiple_user',isAuthenticated,function(req,res){
        if(!req.user.admin) {res.redirect('/');res.flush('Keine Berechtigung');}
        else {
            res.render('create_multiple_user');
        }
    });

    router.post('/create_multiple_user',isAuthenticated,function(req,res){
        if(!req.user.admin) {res.redirect('/');res.flush('Keine Berechtigung');}
        else {
            console.log("Got multiple User: ");console.log(req.body);
            req.body.user.forEach(function (i){
            ...
            res.render('success',{msg: "Nutzer erfolgreich hinzugef\u00fcgt"})
        }
    });
    ...
    router.get('/edit_grm_user/:user_id',isAuthenticated, function(req,res){
        if(!req.user.admin) {res.redirect('/');res.flush('Keine Berechtigung');}
            ...
            // Mitgliedschaft in Fachschaften f�r den zuvor gefunden Nutzer aus der DB anfordern
            ...
            // Mitgliedschaft in Gremiuen f�r den zuvor gefundenen Nutzer aus der DB anfordern
            membership_model.find({user_id: user_id}, function (err, memberships) {
                if (!memberships) res.status(404).send("Keine Gremnienmitgliedschaft");
                    ...
                    console.log("Membership from DB ");console.log(memberships);
        ...
        if(!req.user.admin) {res.redirect('/');}
            ...
            grm_user_model.findById(user_id, function (err, usr) {
                if (err) {
                    ...
                    console.log("speicher ge�nderte Userdaten");
                    console.log(user);
                    //falls Daten ge�ndert wurden, werden diese in usr ge�ndert
                    ...
                    //schreiben der �nderungen in die DB
                    ...
                    req.body.councils.forEach(function(cou) {
                        cou_id = cou._id;
                        ...
                        membership_council_model.findOne({_id: cou_id}, function (err, council) {
                            if (err) {
                                ...
                                if(validator.isAfter(cou.from,cou.to)){
                                ...
                                council.council_id = cou.council_id || council.council_id;
                                council.from = cou.from || council.from;
                                council.to = cou.to || council.to;
                                council.user_id = cou.user_id || council.user_id;
                                console.log("Save council Membership: " + council);
                                //speichern der ge�nderten Fachschaftsmitgliedschaft
                                ...
                                req.body.comm.forEach(function(membership) {
                                    console.log("aus forEach: ");console.log(membership);
                                    var membership_id = membership.membership_id;
                                        ...
                                        membership_model.findOne({_id: membership_id}, function (err, comm) {
                                            if (err) {
                                                ...
                                                res.send('Committee not found');
                                                        ...
                                                        var succ = membership.successor;
                                                        if (!succ) succ = false;

                                                        if(validator.isAfter(membership.from,membership.to)){
                                                            res.status(400).send("no valid committee timeframe");
                                                        }

                                                        //Gremiummitgliedschaft durch ge�nderte Werte erg�nzen
                                                        comm.grem_id = membership.committee || comm.grem_id;
                                                        comm.from = membership.from || comm.from;
                                                        comm.to = membership.to || comm.to;
                                                        comm.reason = membership.reason || comm.reason;
                                                        comm.successor = succ || comm.successor;
                                                        comm.council_id = membership.council_id || comm.council_id ;
                                                        comm.period_id = membership.period_id  || comm.period_id;
                    ...
                    if (err) {
                       ...
                       student_council_model.find({},function(err,cou){
                            if (err) {
                                ...
                                //Rendern der add_membership.jade mit allen Gremien, Amtszeiten, Fachschaften und der User_id, zus�tzlich noch das Modul moment zur Datumskonvertierung
    ...
    //Handle des Posts zum Hinzuf�gen einer Gremienmitgliedschaft
    ...
    //Handle GET Request f�r Faschaftsmitgliedschaft
                    ...
                    // Finden aller Fachschaften
                        student_council_model.find({},function(err,cou){
                            if (err) {
                                ...
                                //Rendern der add_membership_council.jade mit allen Fachschaften der user_id und dem Modul moment f�r die Datumskonvertierung
                                res.render('add_membership_council',{councils:cou,user_id: user_id,moment: moment})
                            }
                        })
    });
//Handle des POSTs zum Hinzuf�gen einer Fachschaftsmitgliedschaft f�r einen User
    router.post('/add_membership_council/:user_id',isAuthenticated, function(req,res){
        if(!req.user.admin) {res.redirect('/');}
            ...
            membershipsave.save(function (err) {
                ...
                //Form-Post-Redirect bei erfolgreichem Hinzuf�gen der Fachschaftsmitgliedschaft mit vorheriger Ausgabe einer Erfolgsmeldung
                    res.render('success', {msg: "Mitgliedschaft erfolgreich hinzugef\u00fcgt"});
            });
        }
    });
  ...
  router.post('/create_grem_user',isAuthenticated, function(req, res){
      if(!req.user.admin) {res.redirect('/');}
          ...
          console.log('\nsave user: ');
          console.log(user);
          req.body.comm.forEach(function (grem){
              //console.log("ID beim Post: "+user._id);

              if(validator.isAfter(grem.from,grem.to))
              {
                  res.status(400).send("Bad committee membership timeframe");
              }
              var membership = new membership_model({
                  grem_id: grem.committee,
                  from: grem.from,
                  to: grem.to,
                  reason: grem.reason,
                  user_id: user._id,
                  council_id: grem.council_id,
                  period_id: grem.period_id,
                  successor: grem.successor || false
          ...
          //Iteration �ber alle Fachschaftsmitgliedschaften
          req.body.councils.forEach(function (council) {
              //console.log("ID beim Post: "+user._id);
                //Anlegen einer neuen Fachschaftsmitgliedschaft

              if(validator.isAfter(council.from,council.to))
              {
              ...
              console.log(council_membership);
          });
...
//Handle GET auf /create_committee
    router.get('/create_committee',isAuthenticated, function(req, res){
        //Rendern der create_committee.jade nach vorheriger Authentizit�tspr�fung
        if(!req.user.admin) {res.redirect('/');}
        else {
    ...
    router.post('/edit_student_council/:_id',isAuthenticated, function(req, res) {
        ...
        if(!req.user.admin) {res.redirect('/');}
            ...
            //Die zu �ndernde Fachschaft aus der DB holen
            student_council_model.findById(council_id, function (err, cou) {
                if (err) {
                    ...
                    //�ndern des Namens der Fachschaft
    ...
    //Handle POST Request zum �ndern einer Amtszeit
    router.post('/edit_period/:_id',isAuthenticated, function(req, res) {
        ...
        if(!req.user.admin) {res.redirect('/');}
            ...
            //Finden der zu �ndernden Amtszeit aus der DB
            ...
            //Finden aller User
              grm_user_model.find().lean().exec(function (err, grm_users) {
                  if (err) {
                    ...
                    //Rendern von admin.jade mit allen Usern, dem akutellem User, allen Gremien, allen Fachschaften, allen Amtszeiten und dem Modul moment f�r die Datumskonvertierung
                      res.render('admin', {
  ...
  router.get('/signout', function(req, res) {
    ...
    //Handle des Post Request f�r die Suchanfrage
    router.post('/search',isAuthenticated, function(req, res) {
        ...
        membership_model.find(query,function(err,mem) {
            if (err) {
                ...
                query = {};
                //Suchquery f�r die Usersuche erstellen
                ...
                //Finden der User, welche den Suchquery erf�llen
